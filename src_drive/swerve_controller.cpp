...
void SwerveController::InitMotorPins(int fwdDir, int fwdPwm, int encDir,
                                     int encPwm) {
  this->fwdDir = fwdDir;
  this->fwdPwm = fwdPwm;
  this->encDir = encDir;
...
void SwerveController::InitSensorPins(int rst, int wrot, int enc1, int enc2,
                                      int gyroSCL, int gyroSDA, int gyroSDO,
                                      int gyroNCS) {
  this->resetHS = rst;
  this->wheelRot = wrot;
  this->enc1 = enc1;
...
void SwerveController::updatePos() {
  ...
  if (abs(temp_dx) > 0 || abs(temp_dy) > 0) {
    ...
    this->instantaneousSpeed = sqrt(temp_dx * temp_dx + temp_dy * temp_dy) /
                               (currentMillis - millisLastPolled);
...
void SwerveController::updateTarget(SwerveUpdateData &data) {
  ...
  this->targetAngleSpeed =
      sqrt(rx * rx + ry * ry) / 256.0 + sqrt(lx * lx + ly * ly) / 256.0;
  this->targetAngle = (lx != 0.0) ? atan(ry / rx) : 3.1415 / 4.0;
}

void encMotChange() {
  if (digitalRead(swerve.enc2) == HIGH) {
    swerve.angle += 2 * 3.14159 * swerve.ENC_MOT_GEAR_RATIO /
                    swerve.ENC_PULSES_PER_ROTATION;
  } else {
    swerve.angle -= 2 * 3.14159 * swerve.ENC_MOT_GEAR_RATIO /
                    swerve.ENC_PULSES_PER_ROTATION;
  }
